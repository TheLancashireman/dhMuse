...
#
# Usage:
#  cd-to-ogg.pl xml-file [config-file]
#
# Rips the CD to separate ogg files, one per track, as specified in the xml-file
#
...
if ( ! defined $xmlfile )
{
	print STDERR "Usage: cd-to-ogg.pl xml-file [config-file]\n";
	exit 0;
}
...
foreach $vol ( sort keys %xml_disclist )
{
	...
	while ( defined $xml_tracklist{$vol . ":" . $trkno} )
	{
		...
		cd_to_ogg($trkno, $xml_tracklist{$track});
